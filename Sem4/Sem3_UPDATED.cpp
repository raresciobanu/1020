...
void scriereTelefoaneInFisierBinar(FILE* fisier, telefon t) {
	...
	char aux[100]; //100 * sizeof(char) = 100 * 1 = 100bytes
	strcpy(aux, t.marca); //"Huawei\0" ===   6 * 1 + 1byte de la \0 = 7 bytes

	//7 (6+1)
	//Huawei
	int dim = strlen(aux) + 1;
	...
	strcpy(aux, t.proprietarTelefon.nume);
	dim = strlen(aux) + 1;
	fwrite(&dim, sizeof(dim), 1, fisier);
	fwrite(aux, dim, 1, fisier);
	fwrite(&t.proprietarTelefon.varsta, sizeof(t.proprietarTelefon.varsta), 1, fisier);
...
void citireTelefoanDinFisierBinar(FILE* fisier, telefon& t) {
	...
	t.marca = (char*)malloc(dim * sizeof(char));
	strcpy(t.marca, aux);
	...
	fread(&dim, sizeof(dim), 1, fisier);
	fread(t.proprietarTelefon.nume, dim, 1, fisier);
	fread(&t.proprietarTelefon.varsta, sizeof(t.proprietarTelefon.varsta), 1, fisier);
...
void main() {
	...
	while (fscanf(fisierTxt, "%s", &aux) == 1) {
		printf("%s\n", aux);
	}
